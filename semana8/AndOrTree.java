...
/**
 * Builds an AndOrTree from a list of sub-formulas
 * 
...
public class AndOrTree {
    ...
    public AndOrTree(List<Formula> subFormulas) {
    	ArrayList<TreeNode> childs = new ArrayList<>();	//creates new array list of the childs pf the tree
    	for (Formula f: subFormulas) {
    		this.root = new TreeNode(f.getName(), f.getFormula(), childs);
    		childs.add(this.root);	//adds the root to the childs
    	}
    }
    ...
    public List<AndOrTree> getSubtrees() {
    	List<AndOrTree> result = new ArrayList<AndOrTree>();
    	for (TreeNode t: this.root.childNodes) {
    		AndOrTree tree = new AndOrTree (t);		//creates a new subtree
    		result.add(tree);	//adds the substrees to the respective list of AndOrTrees
    	}
        return result;
    }
    ...
    public List<String> getLeafIds() {
    	...
    	for (TreeNode t: this.root.childNodes) {
    		AndOrTree tree = new AndOrTree (t);
    		if(t.type == NodeType.LEAF) {
    			result.add(t.id);	//adds list of ids for the nodes to the set
    		} else {
    			result.addAll(tree.getLeafIds());	//adds the ids of all leaf nodes
    		}
    	}
    ...
    public Boolean allLeafs(List<SymbolValue> valuation) {
       List<String> allLeafs = getLeafIds();
       int count = 0;
       for (String x: allLeafs) {
    	   for (SymbolValue sv: valuation) {
    		   if (sv.getSymbol().equals(x)) {
    			   count++;
    			   break;
    		   }
    	   }
       }
       if (count == allLeafs.size()) {
    	   return true;
       } else {
    	   return false;
       }
    ...
    public Boolean evaluate(List<SymbolValue> valuation) {
    	List<Boolean> values = new ArrayList<Boolean>();	//creates list of boolean values
    	for (TreeNode t: this.root.childNodes) {
    		if (t.type == NodeType.LEAF) {
    			values.add(getValue(t.id, valuation));	//add the value of the id in the valuation
    		} else {
    			values.add(new AndOrTree(t).evaluate(valuation));
    		}
    	}
    	if (this.root.type == NodeType.AND) {
    		return !values.contains(false);
    	} else if (this.root.type == NodeType.OR) {
    		return values.contains(true);
    	} else if (this.root.type == NodeType.NOT) {
    		return !values.get(0);
    	}
		return null;
    } 
